...
from models import db, User, ParkingLot, ReserveSpot, ParkingSpot
from flask_mail import Message
from app import app, mail   # ✅ important: use the same mail object
import csv
import os
...
def send_daily_reminders(self):
    try:
        with app.app_context():
            ...
            for user in users:
                subject = "🚗 Friendly Parking Reminder – We Miss You at ParkEase!"
...
<div style="font-family: Arial, sans-serif; background:#f5f7fa; padding:20px;">
  <div style="max-width:600px; margin:auto; background:white; padding:25px; border-radius:10px; box-shadow:0 3px 10px rgba(0,0,0,0.1);">
      ...
      <h2 style="color:#4568dc; text-align:center; margin-top:0;">
          Hello {user.username} 👋
      </h2>

      <p style="color:#333; line-height:1.6;">
          We noticed you haven’t booked a parking spot recently.  
          No worries — we're here to help you get the best parking experience!
      </p>

      <h3 style="color:#4568dc; margin-top:25px;">✨ Recommended Parking Spots</h3>

      <ul style="padding-left:20px; color:#444;">
                """

                for lot in new_lots[:3]:
                    html_body += f"""
        <li style="margin-bottom:10px;">
            <strong>{lot.location_name}</strong><br>
            <span style="color:#777;">{lot.address}</span><br>
            <span style="color:#4568dc;">₹{lot.price}/hour</span>
        </li>
                    """
      ...
      <a href="http://localhost:5173" 
         style="display:inline-block; background:#4568dc; color:white; padding:12px 20px; border-radius:6px; text-decoration:none; font-size:15px;">
          Book a Spot Now →
      </a>

      <p style="color:#777; font-size:13px; text-align:center; margin-top:35px;">
          ParkEase — Smart Parking Solutions
      </p>
  </div>
</div>
    ...
    try:
        with app.app_context():
            ...
            users = (
                ...
                .join(ReserveSpot)
                .filter(ReserveSpot.parking_time >= first_day)
                .distinct()
                .all()
            )

            # If no user has bookings this month, send to a few users for goodwill
            if not users:
            ...
            for user in users:
                ...
                total_spent = sum(float(r.parking_cost or 0) for r in reservations if r.leaving_time)
                total_hours = sum(
...
<div style="font-family: Arial, sans-serif; background:#f5f7fa; padding:20px;">
  <div style="max-width:600px; margin:auto; background:white; padding:25px; border-radius:10px; box-shadow:0 3px 10px rgba(0,0,0,0.1);">
    ...
    <h2 style="color:#4568dc; text-align:center; margin-top:0;">📊 Monthly Parking Report</h2>
    <p style="color:#444; text-align:center;">{now.strftime('%B %Y')} Summary for <strong>{user.username}</strong></p>

    <div style="display:flex; justify-content:space-between; text-align:center; margin-top:25px;">
      <div><strong style="font-size:22px; color:#4568dc;">{total_bookings}</strong><br><span style="font-size:13px;color:#666;">Bookings</span></div>
      <div><strong style="font-size:22px; color:#4568dc;">₹{total_spent:.2f}</strong><br><span style="font-size:13px;color:#666;">Spent</span></div>
      <div><strong style="font-size:22px; color:#4568dc;">{total_hours:.1f}h</strong><br><span style="font-size:13px;color:#666;">Hours Parked</span></div>
    </div>

    <p style="color:#333; line-height:1.6; margin-top:30px;">
      Thanks for choosing ParkEase this month 😊  
      We’re glad to have you with us — and we’re committed to making parking easy, fast, and reliable.
    </p>

    <a href="http://localhost:5173"
       style="display:block; width:fit-content; margin:20px auto; background:#4568dc; color:white; padding:12px 20px; border-radius:6px; text-decoration:none; font-size:15px;">
...
"""
            ...
            print(f"✅ Monthly report emails sent: {sent_count}")
...
# ✅ EXPORT USER PARKING HISTORY (CSV attachment)
@celery.task(bind=True)
    ...
    try:
        with app.app_context():
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as file:
                ...
                writer.writerow(["Reservation ID", "Spot ID", "Start Time", "End Time", "Cost"])
                for r in reservations:
                    writer.writerow([
                        r.id, r.spot_id,
                        r.parking_time, r.leaving_time or "Active",
                        r.parking_cost
                    ])

            subject = "Your Parking History Export is Ready"
            body = f"Hello {user.username},\n\nYour parking data export is attached.\n\n- ParkEase Team"
...
Your parking session has ended and your spot is released.

Location: {parking_lot.location_name}
Spot: {parking_spot.id}
End Time: {reservation.leaving_time}

Thanks for using ParkEase!
"""

            msg = Message(subject=subject, recipients=[user.email], body=body)
            mail.send(msg)

            print(f"✅ Release email sent to {user.email}")
    except Exception as e:
        print(f"❌ Release email error: {e}")
